    ...
    <header className="bg-white/80 backdrop-blur-sm sticky top-0 z-50 border-b">
      ...
      <div className={`fixed inset-0 z-50 pointer-events-none ${open ? "pointer-events-auto" : ""}`}>
        ...
        <aside
          ...
          aria-modal="true"
          aria-hidden={!open}
          className={`fixed top-0 right-0 h-full w-72 sm:w-80 bg-white shadow-xl transform transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"}`}
        >
          ...
          <div className="p-4 space-y-3">
            ...
            <Link to="/shop" onClick={() => setOpen(false)} className="block mt-3 w-full text-center rounded-md bg-emerald-600 text-white px-4 py-2 text-sm font-semibold hover:bg-emerald-700">
              Shop
            </Link>
          </div>
        </aside>
